    ...
    stages {
        ...
        stage('Deploy to Production') {
            ...
            steps {
                withCredentials([usernamePassword(credentialsId: 'docker_hub_login', passwordVariable: 'docker_hub_loginPassword', usernameVariable: 'docker_hub_loginUser' )]) {
                    script {
                        sh "sshpass -p '$USERPASS' -v ssh -o StrictHostKeyChecking=no vagrant@192.168.56.60 \"docker pull platof/my-react-app:${env.BUILD_NUMBER}\""
                        try {
                        ...
                        sh "sshpass -p '$USERPASS' -v ssh -o StrictHostKeyChecking=no vagrant@192.168.56.60 \"docker run --restart always --name my-react-app -p 3000:3000 -d platof/my-react-app:${env.BUILD_NUMBER}\""
                    }
                }
            }
        }
